...
This repository contains a single Jupyter notebook, `forecasting.ipynb`, that walks through a full time series analysis workflow on the [Daily Oil Price dataset](https://www.kaggle.com/datasets) (WTI crude oil price, `oil.csv`). It covers:
...
Missing values are handled via `na_values = ['na', '-', '.', '']` on read and dropped where needed for modeling. If the dataset isn't included in this repo, download it from Kaggle and place `oil.csv` in the repo root before running the notebook.
